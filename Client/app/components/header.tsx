...
export function Header({ onRoomCreated, onRoomSelect }: HeaderProps) {
  const { data: session } = useSession();
  const navigate = useNavigate();
  const [state, dispatch] = useReducer(reducer, initialState);
  const menuRef = useRef<HTMLDivElement>(null);

  const { showUserMenu, loggingOut, showModal, roomName, creating, createError } = state;
  ...
  const handleLogout = async () => {
    ...
    await signOut();
    dispatch({ loggingOut: false, showUserMenu: false });
    navigate("/login");
  };

  const closeModal = () => {
    dispatch({ showModal: false, roomName: "", createError: "" });
  };

  const createRoom = async () => {
    if (!roomName) return;
    dispatch({ creating: true, createError: "" });
    ...
    if (!res.ok) {
      dispatch({ createError: data.error ?? "Faild to create room" });
      return;
    }
  ...
  return (
    <>
      <nav className="h-14 bg-[#13161b] border border-[#232830] flex items-center px-5 gap-1.5 shrink-0">
        <Link
          to="/"
          className="flex items-center gap-2 mr-3.5 text-[#e8ff47] font-extrabold text-lg tracking-[-0.5px] no-underline"
        >
        ...
        <div className="flex items-center gap-2 ml-auto">
          {session ? (
            <>
              <Notifications onRoomSelect={onRoomSelect} />
              <button
                onClick={() => dispatch({ showModal: true })}
                className="flex items-center gap-1.5 text-[13px] font-semibold text-[#13161b] bg-[#e8ff47] hover:bg-[#d4eb3a] cursor-pointer ml-1.5 rounded-[7px] py-1.5 px-3 transition-colors"
              >
                <svg
                  width="11"
                  height="11"
              ...
              <div className="relative" ref={menuRef}>
                <button
                  onClick={() => dispatch({ showUserMenu: !showUserMenu })}
                  className={`flex items-center gap-2 py-1 px-2.5 border border-[#232830] cursor-pointer rounded-[20px] transition-colors ${showUserMenu
                      ? "bg-[#20252e]"
                      : "bg-[#1a1e25] hover:bg-[#20252e]"
                    }`}
                >
            ...
            <>
              <Link
                to="/login"
                className="text-sm text-[#a0a8b8] hover:text-[#eef0f4] transition-colors px-2 py-1"
              >
                Login
              </Link>
              <Link
                to="/register"
                className="text-sm font-semibold text-[#13161b] bg-[#e8ff47] hover:bg-[#d4eb3a] transition-colors px-3 py-1.5 rounded-lg"
              >
                Register
              </Link>
            </>
      ...
      {showModal && (
        ...
        >
          <div className="bg-[#13161b] border border-[#232830] rounded-2xl p-6 w-85 shadow-2xl shadow-black/50">
            <div className="flex items-center justify-between mb-1">
              <h3
                className="text-[15px] font-bold text-[#eef0f4]"
            ...
            <p className="text-xs text-[#4e5668] mb-5">Pick a name.</p>

            <input
              placeholder="e.g. dev-backend"
              value={roomName}
              onChange={(e) => dispatch({ roomName: e.target.value })}
